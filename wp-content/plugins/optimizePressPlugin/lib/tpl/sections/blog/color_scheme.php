<div class="op-bsw-grey-panel-content op-bsw-grey-panel-no-sidebar">
	...
	if ($type=='font'){
		?>
		<li>
			<label for="op_sections_<?php echo $name ?>" class="form-title"><?php _e($title,OP_SN) ?></label>
			<?php
		...
		?>
		<li>
			<label for="op_sections_color_scheme_field_<?php echo $name ?>" class="form-title"><?php _e($title,OP_SN) ?></label>
			<?php echo (empty($help) ? '':'<p class="op-micro-copy">'.__($help,OP_SN).'</p>'); op_color_picker('op[sections][color_scheme_fields]['.$name.']',op_default_attr('color_scheme_fields',$name),'op_sections_color_scheme_field_'.$name); ?>
		</li>
		<?php
	}
	?>
