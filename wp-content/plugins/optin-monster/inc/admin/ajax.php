		...
		switch ( $provider ) :
		    ...
		    case 'feedblitz' :
				if ( array_key_exists( $provider, $providers ) && isset( $providers[$provider][$email_id] ) ) :
    				...
    				$api  = wp_remote_get( 'https://www.feedblitz.com/f.api/syndications?key=' . $providers['feedblitz'][$email_id]['api'], array( 'headers' => array( 'Content-Type' => 'application/xml', 'User-Agent' => 'OptinMonster API' ) ) );
    				$code = wp_remote_retrieve_response_code( $api );
    				if ( 200 != $code ) {
        				echo json_encode( array( 'error' => wp_remote_retrieve_response_message( $api ) ) );
    					die;
    				}

    				libxml_use_internal_errors( true );
    				$lists = json_decode( json_encode( simplexml_load_string( wp_remote_retrieve_body( $api ) ) ) );
    				if ( ! $lists ) {
        				echo json_encode( array( 'error' => __('Unable to authenticate to the Feedblitz API. Please try again.','optin-monster') ) );
    					die;
					} else {
						// Send back necessary HTML for the user to make a selection for the list to subscribe to.
						$ret .= '<div id="om-email-lists"><p class="padding-top"><strong>Sweet - we are connected! Select a list from the options below. Successful optins will be subscribed to the selected list.</strong></p>';
						$ret .= '<select id="om-email-list" name="email_list">';
    					foreach ( $lists->syndications->syndication as $list ) :
    						$ret .= '<option value="' . $list->id . '">' . $list->name . '</option>';
    					endforeach;
    					$ret .= '</select></div>';
					}
	...
	public function connect_email() {
		...
		switch ( $provider ) :
		    ...
		    case 'feedblitz' :
				...
				// Load the Feedblitz API.
				$api  = wp_remote_get( 'https://www.feedblitz.com/f.api/syndications?key=' . $data['api_key'], array( 'headers' => array( 'Content-Type' => 'application/xml', 'User-Agent' => 'OptinMonster API' ) ) );
				$code = wp_remote_retrieve_response_code( $api );
				if ( 200 != $code ) {
    				echo json_encode( array( 'error' => wp_remote_retrieve_response_message( $api ) ) );
					die;
